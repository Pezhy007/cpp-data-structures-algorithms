...
class Heap {
    ...
    public:
        ...
        void insert(T element) {
            ...
            while (currentIndex > 1 && this->tree[currentIndex] < this->tree[getParentPosition(currentIndex)]) {
                std::swap(this->tree[currentIndex], this->tree[getParentPosition(currentIndex)]);
                currentIndex = getParentPosition(currentIndex);
            }
        }
        
        void remove(T value) {
            heapIndex indexToRemove = std::find(this->tree.begin(), this->tree.end(), value) - this->tree.begin();
            
            if (indexToRemove == this->tree.size()) {
                return;
            }
            
            this->tree[indexToRemove] = this->tree.back();
            this->tree.pop_back();
            
            heapifyDown(indexToRemove);
            
            heapIndex parentIndex = getParentPosition(indexToRemove);
            if (indexToRemove > 1 && this->tree[indexToRemove] < this->tree[parentIndex]) {
                while (indexToRemove > 1 && this->tree[indexToRemove] < this->tree[parentIndex]) {
                    std::swap(this->tree[indexToRemove], this->tree[parentIndex]);
